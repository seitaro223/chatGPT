%% BT08A1_Fform_area_recalc_audit_v3.m
...
for b = bundles
    ...
    if height(xeqSummary) == 0 || ~ismember("Bundle", string(xeqSummary.Properties.VariableNames))
        warning("No usable xeqSummary table. Bundle %d skipped.", b);
        continue;
    end
    ...
    for i = 1:height(X)
        ...
        if ismember("numerator_r3", string(X.Properties.VariableNames))
            row.numerator_r3 = X.numerator_r3(i);
            row.denominator_r3 = X.denominator_r3(i);
        end
        if ismember("reading_method", string(X.Properties.VariableNames))
            row.xeq_reading_method = X.reading_method(i);
        end
...
function X = readXeqSummary(file)
    % Robust reader for 02_xeq_recalc.
    %
    % r3 workbook structure is human-made and may not be a clean table.
    % Known structure:
    %   Row 4:
    %     A: Bundle
    %     B: z_ratio
    ...
    %     E: z_heat_ratio
    %     F: 備考
    %   Rows 5:9 contain bundle-level anchor rows.
    %
    % Instead of relying only on header text, scan rows where:
    %   col A = 108/161/164,
    %   col B = z_ratio in [0,1.2],
    %   col E = z_heat_ratio in [0,1.2].
    %
    % This preserves the user's manual decisions:
    %   - linear interpolation
    %   - stopping before / including DNB location
    %   - using the r3 z_ratio and z_heat_ratio as human-made anchor values.
    ...
    X.reading_method = strings(0,1);

    rows = [];

    % Primary: numeric scan of first summary block.
    for i = 1:size(C,1)
    ...
    % Keep only the first summary block before detailed table starts.
    % The detailed table also has Bundle in col A and many rows, but col E is Hlg,
    % so zheat<=1.2 usually filters it out. This extra guard is for safety.
    if ~isempty(rows)
        rows = rows(rows < 20);
    end

    if isempty(rows)
        warning("02_xeq_recalc summary rows not found. Fallback: use one z_ratio anchor per bundle from hard-coded known r3 rows if possible.");
        return;
    end
    ...
    for k = 1:numel(rows)
        ...
        newRow.reading_method = "numeric_scan_02_xeq_recalc_summary_block";
