%% BT09_0_macro_Fform_input_location_audit.m
% BT09-0：マクロブック側の F_form 入力位置を監査する
%
% 目的：
%   BT08-A3で確定したF_form差し替え表を、マクロブックのどこへ入れるべきか確認する。
%
%   この段階では、マクロブックを編集しない。
%   元マクロブックを上書きしない。
%   F_form列、関連ヘッダ、関連式、No列、シート名を調べ、
%   「差し替え可能そうな候補位置」をMarkdownログとして出す。
%
% 入力：
...
%
% 出力：
%   - BT09_0_macro_Fform_input_location_audit_yyyymmdd_HHMMSS.xlsx
%   - run_report_BT09_0_macro_Fform_input_location_audit_yyyymmdd_HHMMSS.md
%
% 次：
%   BT09-A：
%     マクロブックをコピーし、F_form列だけFform_linearへ差し替える。
%
% 注意：
%   - 本スクリプトは監査専用。
%   - マクロブックの値は一切変更しない。
...
outXlsx = "BT09_0_macro_Fform_input_location_audit_" + timestamp + ".xlsx";
outMd   = "run_report_BT09_0_macro_Fform_input_location_audit_" + timestamp + ".md";
...
for sh = macroSheets(:)'
    ...
    if isempty(C)
        ...
        inv.has_F_form_like = any(contains(normalizeText(cellToStringArray(C)), "fform") | contains(normalizeText(cellToStringArray(C)), "f_form"));
        inv.has_No_like = any(normalizeText(cellToStringArray(C)) == "no");
    end
    ...
    % Header search in first 30 rows
    headerRowsMax = min(size(C,1), 30);
    ...
    % Formula/string search over whole used range for F_form / Fform / qM / qP / PM
    for r = 1:size(C,1)
        for c = 1:size(C,2)
            s = cellToString(C{r,c});
            ns = normalizeText(s);
            ...
            if contains(ns, "fform") || contains(ns, "f_form") || contains(ns, "pm") || contains(ns, "qp") || contains(ns, "qm")
...
for es = expectedSheets(:)'
    ...
    row.contains_match_count = sum(contains(macroSheets, es) | contains(es, macroSheets));
    row.contains_matches = strjoin(macroSheets(contains(macroSheets, es) | contains(es, macroSheets)), ", ");
...
% Candidate logic:
%   1. Same sheet name exists in macro workbook.
%   2. F_form-like header exists.
%   3. No-like header exists near same header row.
%   4. Data rows likely start at header row + 1.
%   5. A3 excel_row_if_header1 may be directly usable if header is row 1 in macro sheet.
%
% If exact sheet names do not exist, provide candidates based on header hits only.
...
for i = 1:height(fFormHeaderCandidates)
    ...
    cand.header_row_offset = rowOffset;
...
qc = makeQC(sheetInventory, fFormHeaderCandidates, noHeaderCandidates, replacementLocationCandidates, sheetNameMatches, replaceMap);
...
md(end+1) = "# BT09-0 macro F_form input location audit";
md(end+1) = "";
...
md(end+1) = "";
md(end+1) = "```text";
md(end+1) = "- このBT09-0は監査専用であり、マクロブックを編集していない。";
md(end+1) = "- F_form候補列とNo候補列の位置が確認できたら、BT09-Aでコピー版マクロブックへの差し替えを行う。";
md(end+1) = "- もしF_form候補が見つからない場合は、マクロブックの入力シート名・列名がcurrent_bundle_inputと異なる可能性がある。";
md(end+1) = "```";
...
function latest = findLatestMacroBook()
    ...
    % Prefer Celata workbook names, otherwise newest .xlsm
    names = string({d.name});
    preferred = contains(names, "celata", "IgnoreCase", true) | contains(names, "セル", "IgnoreCase", true) | contains(names, "櫻井", "IgnoreCase", true);
...
function T = normalizeReplaceMap(T)
    ...
    % Alias back to old names if needed
...
function qc = makeQC(sheetInventory, fHits, noHits, locCand, sheetNameMatches, replaceMap)
    item = strings(0,1);
    status = strings(0,1);
    value = strings(0,1);
    reading = strings(0,1);
