...
% sheetnames can return row/column arrays depending on MATLAB version.
% Also, "for s = sheets'" over a column vector can pass the whole column at once.
% Therefore loop by index and force scalar sheet names.
availableSheets = string(sheetnames(inputFile));
availableSheets = availableSheets(:);

for ii = 1:numel(sheets)
    s = string(sheets(ii));
    if ~any(strcmp(availableSheets, s))
        continue;
    end
    T = readtable(inputFile,'Sheet',char(s),'VariableNamingRule','preserve');
