%% ST_BT05_single_tube_xeq_independent_effect_v3.m
% ST-BT05 v3：単管側における x_eq 独立効果の切り分け診断（修正版）
%
% v1の問題：
...
%
% v3の方針：
%   - v2では r8 の tm_F1_ST だけを読んだため Table10 86行に落ちた。
%   - v3では r8 の tm_r124_F1_T8_14 を読み、Table8〜14全体の hlg を結合する。
...
f1Sheet = "tm_r124_F1_T8_14";

ts = string(datetime("now","Format","yyyyMMdd_HHmmss"));
outXlsx = "ST_BT05_single_tube_xeq_independent_effect_v3_" + ts + ".xlsx";
outMd   = "run_report_ST_BT05_single_tube_xeq_independent_effect_v3_" + ts + ".md";
...
md = strings(0,1);
md(end+1) = "# ST-BT05 v3 single-tube x_eq independent-effect diagnostic";
md(end+1) = "";
md(end+1) = "作成日時: " + string(datetime("now"));
md(end+1) = "";
md(end+1) = "## 1. 目的";
md(end+1) = "";
md(end+1) = "ST-BT05 v1では、current_single_tube入力だけではHsub_trueとx_eq計算に必要な列が不足し、x_eq独立効果を確認できなかった。ST-BT05 v2では、hlg結合元にtm_F1_STを使ったためTable10 86行に落ち、Table9〜12比較として不十分だった。";
md(end+1) = "v3では、v10のtarget_rows_T8_12とr8のtm_r124_F1_T8_14をNo_TableNoで結合し、qP_F1側の熱収支からx_eqを計算して、単管側のx_eq独立効果を確認する。";
md(end+1) = "";
...
md(end+1) = "";
md(end+1) = "```text";
md(end+1) = "1. ST-BT05 v3の結果をworking_logへ追記する。";
...
try
    ...
    % Use pinv to avoid MATLAB rank-deficiency warnings from backslash.
    % Rank deficiency is expected here because Hsub, P, Tsub and x_eq can be strongly collinear.
    beta = pinv(Xok) * yok;
    ...
    if fit.rankX < fit.k
        fit.status = "OK_rank_deficient_diagnostic_only";
    else
        fit.status = "OK";
    end
catch ME
...
add("model_OK_count", ok(sum(startsWith(modelSummary.status,"OK"))>=8), sprintf("%d/%d",sum(startsWith(modelSummary.status,"OK")),height(modelSummary)), "主要モデル実行数。");
...
add("task_status", "CHECK_AFTER_RUN", "ST-BT05_v3", "数値結果を見て採否を判断する。");
add("v1_policy", "not_adopt", "missing_Hsub_xeq", "ST-BT05 v1はx_eq独立効果を計算できず、v2はTable10 86行に落ちた。v3を採用候補とする。");
...
add("next_after_STBT05_v3", "summarize_for_user_and_Claude", "personal_summary", "ST-BT05 v3後に全体を私用まとめ＋Claudeレビュー依頼文へ整理する。");
