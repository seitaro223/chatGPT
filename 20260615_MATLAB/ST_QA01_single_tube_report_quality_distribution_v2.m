%% ST_QA01_single_tube_report_quality_distribution_v2.m
...
%% User settings
outPrefix = 'ST_QA01_single_tube_report_quality_distribution_v2';
...
%% TableNo parse
% readtableがTableNo等をcell/stringとして読む場合があるため、必ず数値化関数を通す。
if isempty(colTable)
    ...
    T.TableNo_QA = local_parse_table_no(T.(colTable));
end

%% Extract variables
% Excel由来でcell/string/categorical混在になっても落ちないように、全部local_to_doubleで読む。
T.report_quality = local_to_double(T.(colQ));
if ~isempty(colLD);   T.LD_QA = local_to_double(T.(colLD)); else; T.LD_QA = nan(height(T),1); end
if ~isempty(colTsub); T.Tsub_QA = local_to_double(T.(colTsub)); else; T.Tsub_QA = nan(height(T),1); end
if ~isempty(colPM);   T.PM_QA = local_to_double(T.(colPM)); else; T.PM_QA = nan(height(T),1); end
if ~isempty(colP);    T.P_MPa_QA = local_to_double(T.(colP)); else; T.P_MPa_QA = nan(height(T),1); end
if ~isempty(colHsub); T.Hsub_QA = local_to_double(T.(colHsub)); else; T.Hsub_QA = nan(height(T),1); end
...
function y = local_to_double(x)
    % Robust conversion from Excel-read table column to double vector.
    % Handles numeric, cell, string, char, categorical, and values containing commas/spaces.
...
function tableNo = local_parse_table_no(x)
    % TableNo may be numeric, cell, string, or labels such as 'Table9'/'T9'.
    if isnumeric(x) || islogical(x)
