%% T10R01b_Table10_legacy_scope_TmTsat_audit_v1.m
...
runName = 'T10R01b_Table10_legacy_scope_TmTsat_audit_v1';
ts = datestr(now, 'yyyymmdd_HHMMSS');
outXlsx = sprintf('T10R01b_table10_legacy_scope_TmTsat_audit_v1_%s.xlsx', ts);
outMd   = sprintf('run_report_T10R01b_table10_legacy_scope_TmTsat_audit_v1_%s.md', ts);
    ...
    for fidx = 1:numel(files)
        ...
        for sidx = 1:numel(sh)
            ...
            if ~isempty(matchedIDs)
                ...
                tmp.match_min_expt = min(matchedIDs);
                tmp.match_max_expt = max(matchedIDs);
