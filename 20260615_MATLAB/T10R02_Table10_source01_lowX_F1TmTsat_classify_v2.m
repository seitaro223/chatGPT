%% T10R02_Table10_source01_lowX_F1TmTsat_classify_v2
% Purpose:
...
%
% Outputs:
%   T10R02_Table10_source01_lowX_F1TmTsat_classify_v2_YYYYMMDD_HHMMSS.xlsx
%   run_report_T10R02_Table10_source01_lowX_F1TmTsat_classify_v2_YYYYMMDD_HHMMSS.md
%
% Notes:
...
scriptName = 'T10R02_Table10_source01_lowX_F1TmTsat_classify_v2';
rootDir = fileparts(mfilename('fullpath'));
if isempty(rootDir); rootDir = pwd; end
cd(rootDir);

ts = datestr(now, 'yyyymmdd_HHMMSS');
outXlsx = fullfile(rootDir, sprintf('T10R02_Table10_source01_lowX_F1TmTsat_classify_v2_%s.xlsx', ts));
outMd   = fullfile(rootDir, sprintf('run_report_T10R02_Table10_source01_lowX_F1TmTsat_classify_v2_%s.md', ts));
...
function [legacyScan, legacyIDsAnchor, tmByMode, sheetScanLog, colDetectLog] = scanWorkbooks(rootDir, rawIDs, outXlsx)
    ...
    for f = 1:numel(files)
        ...
        if startsWith(nm, 'T10R02_Table10_source01_lowX_F1TmTsat_classify_v2_'); continue; end
        try
            shs = sheetnames(fp);
        catch
            continue;
