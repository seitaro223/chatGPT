%% T10R02_Table10_source01_lowX_F1TmTsat_classify_v3
% Purpose:
...
%
% Outputs:
%   T10R02_Table10_source01_lowX_F1TmTsat_classify_v3_YYYYMMDD_HHMMSS.xlsx
%   run_report_T10R02_Table10_source01_lowX_F1TmTsat_classify_v3_YYYYMMDD_HHMMSS.md
%
% Notes:
...
scriptName = 'T10R02_Table10_source01_lowX_F1TmTsat_classify_v3';
% Avoid noisy warnings from old workbooks with very long header names.
warning('off','MATLAB:table:ModifiedAndSavedVarnames');
warning('off','MATLAB:table:VariableNamesTruncated');
...
outXlsx = fullfile(rootDir, sprintf('T10R02_Table10_source01_lowX_F1TmTsat_classify_v3_%s.xlsx', ts));
outMd   = fullfile(rootDir, sprintf('run_report_T10R02_Table10_source01_lowX_F1TmTsat_classify_v3_%s.md', ts));
...
function [legacyScan, legacyIDsAnchor, tmByMode, sheetScanLog, colDetectLog] = scanWorkbooks(rootDir, rawIDs, outXlsx)
    ...
    for f = 1:numel(files)
        ...
        if startsWith(nm, 'T10R02_Table10_source01_lowX_F1TmTsat_classify_v'); continue; end
        try
            shs = sheetnames(fp);
        catch
            continue;
        ...
        for si = 1:numel(shs)
            ...
            try
                % Use modified names and suppress long-name warnings.  The original
                % workbooks contain very long Japanese headers; for this audit we
                % only need stable, short MATLAB variable names such as No_TableNo,
                % Tm and Ts.
                opts = detectImportOptions(fp, 'Sheet', sh, 'VariableNamingRule', 'modify');
                X = readtable(fp, opts, 'Sheet', sh, 'VariableNamingRule', 'modify');
            catch
                try
                    X = readtable(fp, 'Sheet', sh, 'VariableNamingRule', 'modify');
                catch
                    continue;
                end
            end
            X = shortenTableVarNames(X);
            ...
            sheetRows = [sheetRows; table(string(files(f).name), sh, mode, isAnchor, nRows, nCols, ...
                listStr(varNames(exptCols)), listStr(varNames(tmCols)), listStr(varNames(tsCols)), ...
            ...
            if isAnchor
                ...
                if ~isempty(idsHere)
                    legacyScan = [legacyScan; table(string(files(f).name), sh, nRows, nCols, numel(idsHere), ...
                        listStr(varNames(exptCols)), minStrNumeric(idsHere), maxStrNumeric(idsHere), ...
...
function X = shortenTableVarNames(X)
    % Make imported workbook variable names short, stable and unique.
    % This avoids namelengthmax issues propagating into logs/output.
    v = string(X.Properties.VariableNames);
    v = regexprep(v, '[^A-Za-z0-9_]', '_');
    for i = 1:numel(v)
        if strlength(v(i)) > 40
            v(i) = extractBefore(v(i), 41);
        end
        if v(i) == "" || isempty(regexp(char(v(i)), '^[A-Za-z]', 'once'))
            v(i) = "V" + sprintf('%03d', i) + "_" + v(i);
        end
    end
    v = matlab.lang.makeUniqueStrings(cellstr(v));
    X.Properties.VariableNames = v;
end

function s = listStr(vals)
    vals = string(vals(:));
...
function mode = detectCalcMode(fileName, sheetName)
    % Robust scalar logicals for MATLAB versions where contains(string array)
    % returns a non-scalar logical array. Also regexp(...,'once') returns
    % [] or a numeric index, so convert it explicitly with ~isempty.
    s = lower(string(fileName) + " " + string(sheetName));
    sh = lower(string(sheetName));

    hasF1F2 = any(contains(s, "f1f2"));
    hasNoF1 = any(contains(s, "nof1")) || any(contains(s, "r123")) || ~isempty(regexp(char(sh), '^tm_st$', 'once'));
    hasF1   = any(contains(s, "f1"))   || any(contains(s, "r124"));
...
function T = addAuditFlags(T, legacyIDsUsed, tmByMode)
    ...
    if ~isempty(tmByMode)
        ...
        for m = 1:numel(modes)
            mode = modes(m);
            sub = tmByMode(string(tmByMode.calc_mode) == mode,:);
            for i = 1:height(sub)
                ...
                switch mode
                    case "noF1"
                        ...
                        T.noF1_Tm_gt_Tsat(row) = logical(sub.Tm_gt_Tsat_any(i));
                        T.noF1_TmMinusTsat_max(row) = sub.Tm_minus_Tsat_max(i);
                    case "F1"
                        T.F1_TmTsat_available(row) = true;
                        T.F1_Tm_gt_Tsat(row) = logical(sub.Tm_gt_Tsat_any(i));
                        T.F1_TmMinusTsat_max(row) = sub.Tm_minus_Tsat_max(i);
                    case "F1F2"
                        T.F1F2_TmTsat_available(row) = true;
                        T.F1F2_Tm_gt_Tsat(row) = logical(sub.Tm_gt_Tsat_any(i));
                        T.F1F2_TmMinusTsat_max(row) = sub.Tm_minus_Tsat_max(i);
                end
            end
        end
    end
