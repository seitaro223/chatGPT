function TM00c_legacy_bridge_audit_v4(rootDir, tm00bXlsx, legacyXlsx, legacySheet)
% TM00c_legacy_bridge_audit_v4
%
% Purpose:
...
assert(~isempty(allD), ['No legacy workbook found. Provide legacyXlsx explicitly, e.g.\n' ...
    '  TM00c_legacy_bridge_audit_v3(pwd, "", "H52Q_current_single_tube_input_v1_....xlsx")']);
...
% Legacy current_single_tube sheets use No_TableNo as a composite key.
% Example observed in ST_F1_T8_14_current:
%   No_TableNo = "9.01_10"
% This means ExptNo = 9.01 and TableNo = 10, not TableNo = 9.
% The separate column No is only a row/index-like number and must not be used
% as ExptNo for the bridge key.
vars = string(T.Properties.VariableNames);
idxComposite = find(strcmpi(vars, "No_TableNo"), 1);

if ~isempty(idxComposite)
    rawCode = string(T.(vars(idxComposite)));
    [exptParsed, tableParsed, parseOK] = parseLegacyNoTableCode(rawCode);
    T.Legacy_No_TableNo_raw = rawCode;

    if any(parseOK)
        T.ExptNo = exptParsed;
        T.TableNo = tableParsed;
        T.TableNo_raw_legacy = rawCode;
    else
        % Fallback for legacy variants where No_TableNo is really a table number.
        ...
        rawTableNo = double(toDoubleLoose(T.TableNo));
        T.TableNo_raw_legacy = rawTableNo;
        T.TableNo = rawTableNo;
        maskCompositeTable = ~isnan(rawTableNo) & rawTableNo >= 8 & rawTableNo < 15;
        T.TableNo(maskCompositeTable) = floor(rawTableNo(maskCompositeTable) + 1e-9);
    end
else
    T = ensureVar(T, 'TableNo', {'TableNo','Table','table','Table_No','TableN','Table_No_'});
    T = ensureVar(T, 'ExptNo', {'ExptNo','EXPT NO','No','NoTableNo','No_Table','No_Table_No','ExpNo','ExperimentNo'});
    T.TableNo_raw_legacy = T.TableNo;
end
...
%% ------------------------------------------------------------------------
function [exptNo, tableNo, ok] = parseLegacyNoTableCode(rawCode)
rawCode = string(rawCode);
exptNo = strings(numel(rawCode), 1);
tableNo = nan(numel(rawCode), 1);
ok = false(numel(rawCode), 1);

for i = 1:numel(rawCode)
    s = strtrim(rawCode(i));
    ...
    % Primary expected form: ExptNo_TableNo, e.g. 9.01_10 or 30.01_12.
    tok = regexp(s, '^([^_]+)_([0-9]+)$', 'tokens', 'once');
    if ~isempty(tok)
        exptNo(i) = string(tok{1});
        tableNo(i) = str2double(tok{2});
        ok(i) = ~isnan(tableNo(i));
        continue;
    end

    % More permissive fallback: first numeric chunk before underscore is ExptNo;
    % first integer after underscore is TableNo.
    tok = regexp(s, '^([^_]+)_.*?([0-9]+)', 'tokens', 'once');
    if ~isempty(tok)
        exptNo(i) = string(tok{1});
        tableNo(i) = str2double(tok{2});
...
%% ------------------------------------------------------------------------
function y = toDoubleLoose(x)
% Robust numeric extraction for legacy columns that may contain strings like
% 'Table9', 'T9', '9.01', or numeric values.  For ordinary numeric cells this
% is identical to double(x).  For text, first try str2double, then extract the
% first numeric token.
if isnumeric(x)
    y = double(x);
    return;
...
for i = 1:numel(s)
    ...
    if ~isnan(yi)
        ...
        tok = regexp(char(si), '[-+]?\d+(?:\.\d+)?', 'match', 'once');
        if ~isempty(tok)
            y(i) = str2double(tok);
        end
    end
